...
namespace QueHub.Service.Services.Notifications;
...
public class SmsSender : ISmsSender
{
    ...
    public SmsSender(IConfiguration config)
    {
        ...
        API_KEY = config["Sms:Sender"]!;
        SENDER = config["Sms:Email"]!;
    ...
    private async Task LoginAsync()
    {
        ...
        // HttpRequestMessage ga Path berish kerak hali yakunlanmagan !!!
    ...
    public async Task<bool> SendAsync(SmsMessage smsMessage)
    {
        ...
        request.Headers.Add("Authorization", $"Bearar {TOKEN}");
        ...
        content.Add(new StringContent("http://1111.uz/test.php"), "callback_url");
        request.Content = content;
        var response = await client.SendAsync(request);
